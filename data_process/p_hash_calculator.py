...
OUTPUT_FILE = "phash_results_data_0.csv"
ERROR_FILE = "error.csv"
MAX_WORKERS = 64

#None = All
LIMIT_DATA_FOLDERS = 1      
...
if __name__ == "__main__":
    ...
    if len(all_tar_tasks) > 0:
        ...
        with open(OUTPUT_FILE, "w") as f_out, open(ERROR_FILE, "w") as f_error:
            f_out.write("image_s3_path,phash\n")
            f_error.write("image_s3_path,phash\n")
            ...
            with Pool(processes=MAX_WORKERS) as pool:
                ...
                for res_list, err_list in pool.imap_unordered(process_single_tar, all_tar_tasks):
                    if res_list:
                        count_ok += len(res_list)
                        f_out.write("\n".join(res_list) + "\n")

                    if err_list:
                        count_ok += len(err_list)
                        f_out.write("\n".join(err_list) + "\n")

                    f_out.flush()
                    f_error.flush()

        print(f"\n\n DONE! Total images: {count_ok}")
        print(f"⏱️ Total time: {time.time() - t_start_process:.2f}s")
        print(f"   - Valid Images: {count_ok} -> saved to {OUTPUT_FILE}")
        print(f"   - Bad Images  : {count_error} -> saved to {ERROR_FILE}")
