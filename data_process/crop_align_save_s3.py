...
def get_s3_client():
    return boto3.client(
        ...
        config=boto3.session.Config(max_pool_connections=50, retries={'max_attempts': 3})
    )
...
def worker_process_person(args):
    person_id, shard_folder, rows = args
    # s3 = get_s3_client()
    global worker_s3
    s3 = worker_s3
...
if __name__ == "__main__":
    ...
    worker_args = []
    skipped_count = 0
    print("[INFO] Preparing Worker Arguments...")
    count = 1
    for pid_key, df_group in tasks_map.items():
        ...
        if count % 10000 == 0:
            print(f"Pid number: {count}")

        start_shard = (pid // SHARD_SIZE) * SHARD_SIZE
        end_shard = start_shard + SHARD_SIZE - 1
        shard_folder = f"person_{start_shard}_{end_shard}"
        worker_args.append((pid, shard_folder, df_group.rows()))
        count += 1
        
    print(f"[RUN] Starting Pool with {NUM_WORKERS} workers for {len(worker_args)} tasks...")
    
    success_count = 0
    with open(CHECKPOINT_FILE, "a") as f_ckpt:
        with Pool(processes=NUM_WORKERS, initializer = init_worker) as pool:
            for pid_done, status in tqdm(pool.imap_unordered(worker_process_person, worker_args), total=len(worker_args)):
                if status == 1:
                    success_count += 1
    ...
    print(f"Persons Uploaded: {success_count}/{len(worker_args)}")
