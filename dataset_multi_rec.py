...
def get_dataloader(
    ...
    # Mxnet RecordIO
        ...
        print("Using MXNet Format")
        train_set = MXFaceDataset(root_dir=root_dir, local_rank=local_rank,use_albumentations=use_albumentations, save_images=save_images, save_dir=save_dir, num_save=num_save)
...
class MXFaceDataset(Dataset):
    def __init__(self, root_dir, local_rank, use_albumentations, save_images=False, save_dir=None, num_save=1000):
        ...
        path_imgrec_1 = os.path.join(root_dir, 'train.rec')
        path_imgidx_1 = os.path.join(root_dir, 'train.idx')
        self.imgrec_1 = mx.recordio.MXIndexedRecordIO(path_imgidx_1, path_imgrec_1, 'r')
        s1 = self.imgrec_1.read_idx(0)
        header, _ = mx.recordio.unpack(s1)
        if header.flag > 0:
            self.header0 = (int(header.label[0]), int(header.label[1]))
            self.imgidx_1 = np.array(range(1, int(header.label[0])))
        else:
            self.imgidx_1 = np.array(list(self.imgrec_1.keys))

        self.label_offset = 2058464
        print(f"REC 1 loaded. Total images: {len(self.imgidx_1)}")

        # # Print for debug
        # rec1_samples, rec1_classes = self.get_class(self.imgrec_1, self.imgidx_1)
        # self.rec1_offset = rec1_classes
        # print(f"Total images rec1: {rec1_samples}")
        # print(f"Total rec1 classes: {rec1_classes}")
        # print(f"Offset rec1: {self.rec1_offset}")

        path_imgrec_2 = os.path.join(root_dir, 'train.rec')
        path_imgidx_2 = os.path.join(root_dir, 'train.idx')
        self.imgrec_2 = mx.recordio.MXIndexedRecordIO(path_imgidx_2, path_imgrec_2, 'r')
        s2 = self.imgrec_2.read_idx(0)
        header2, _ = mx.recordio.unpack(s2)
        if header2.flag > 0:
            self.imgidx_2 = np.array(range(1, int(header2.label[0])))
        else:
            self.imgidx_2 = np.array(list(self.imgrec_2.keys))
        print(f"REC 2 loaded. Total images: {len(self.imgidx_2)}")

        # #Print for debug
        # rec2_samples, rec2_classes = self.get_class(self.imgrec_2, self.imgidx_2)
        # print(f"Total images rec2: {rec2_samples}")
        # print(f"Total rec2 classes: {rec2_classes}")
        
        self.total_indices = []
        self.total_indices.extend([(1, idx) for idx in self.imgidx_1])
        self.total_indices.extend([(2, idx) for idx in self.imgidx_2])

        ## Code save samples for visualization
    ...
    def __getitem__(self, index):
        rec_id, idx_in_rec = self.total_indices[index]

        if rec_id == 1:
            imgrec = self.imgrec_1
            offset = 0
        elif rec_id == 2:
            imgrec = self.imgrec_2
            offset = self.label_offset
        else:
            raise ValueError("Invalid rec_id")

        s = imgrec.read_idx(idx_in_rec)
        ...
        # Add offset
        label = torch.tensor(label + offset, dtype=torch.float32)
